...
- Added direct `pesto::config::*` and `pesto::yenc::*` facade re-exports while
  retaining their legacy public child modules as source-compatibility paths.
  New consumers should use the facades rather than depend on implementation
  layout.
- Kept the memory child modules public for compatibility and retained the
  existing `pesto::memory` facade exports. `memory::alloc` remains a required
  public path because a consuming binary must name `CountingAlloc` in a
